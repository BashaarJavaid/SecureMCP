...
See `ROADMAP.md`. Phases 1 and 2 are complete (items 1–15); Phase 3 is underway — items 16 (Risk Engine v1 + risk decay + human-approval lifecycle), 17 (ABAC conditions, `services/gateway/abac.py`), 18 (richer risk telemetry), 19 (policy versioning: activation snapshots + `policy_versions` rows via `services/gateway/policy_versions.py`, monotonic version stamping, admin rollback endpoint, `--diff-policy`/`--html` on `verify_audit_chain.py`), 20 (Decision Explanation API, `services/gateway/decision_explainer.py`: `GET /admin/decisions/{seq}` reconstructs a past Decision, `POST /admin/decisions/explain` dry-runs the pipeline side-effect-free), and 21 (Policy Simulation Mode, `services/gateway/policy_simulator.py`: `POST /admin/policy/simulate` replays historical decisions against candidate revision snapshots, read-only) are done, and item 22 (architecture diagrams brought up to the implemented system: `ARCHITECTURE.md` §4.1 sequence in §4.2 stage order, §4.4 component diagram mapped to `services/gateway/` modules, §4.5 Compose-MVP + Phase-4 deployment views, §4.6 8-stage data flow with the ABAC split, §4.7 multi-server trust domain discussion — documented, not built) is done too. Next is item 23: the expanded adversarial test suite (risk scoring, ABAC missing-attribute cases incl. inside `not(...)`, explanation/simulation accuracy, the approval-mismatch TOCTOU path, concurrent audit writes).
