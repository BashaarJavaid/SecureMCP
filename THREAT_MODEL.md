...
| Replay of a captured request | **Partial** | The Replay Guard's nonce and timestamp are **client-supplied plaintext** in `params._meta`, bound neither to the request body nor to any secret. This reliably catches accidental resubmission and duplicate delivery. It does **not** stop a capture-and-replay adversary: the API key travels in the `X-SecurMCP-Key` header *of the same request*, so anyone who captured the request captured the credential too, and can mint a fresh nonce and timestamp at will. Note this is currently the worst of both worlds — the nonce is **mandatory**, so a stock MCP client can't call tools at all, yet it buys no protection against the adversary in this row. Phase 5 (item 34) makes the posture per-identity and, for hardened identities, moves the secret **off the wire**: a non-secret key id plus an HMAC over the canonical request, which is what actually closes this |
...
- **v1 fronts a single upstream MCP server.** `UPSTREAM_COMMAND` is one command and `server_id` is one constant (`"default"`), so there is currently no cross-server trust boundary to defend — the policy schema's per-server grants and `"*"` wildcards are forward-compatible structure, not a capability that exists yet. A real server registry is tracked as Phase 5; until it lands, treat any claim about multi-server isolation as unproven.
